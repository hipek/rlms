...
describe Router::Rule::OpenPortsController do
  fixtures :users
  render_views
  ...
  describe "GET index" do
    it "assigns all router_rule_open_ports as @router_rule_open_ports" do
      Router::Rule::OpenPort.stub(:all) { [mock_open_port] }
      get :index
      assigns(:router_rule_open_ports).should eq([mock_open_port])
    end
  end
  ...
  describe "POST create" do
    describe "with valid params" do
      ...
      it "redirects to the created open_port" do
        ...
        response.should redirect_to(router_rule_open_port_url(mock_open_port))
      end
    end
  ...
  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested open_port" do
        Router::Rule::OpenPort.stub(:find).with("37") { mock_open_port }
        mock_router_rule_open_port.should_receive(:update_attributes).with({'these' => 'params'})
  ...
  describe "DELETE destroy" do
    it "destroys the requested open_port" do
      Router::Rule::OpenPort.stub(:find).with("37") { mock_open_port }
      mock_router_rule_open_port.should_receive(:destroy)
      delete :destroy, :id => "37"
    end

    it "redirects to the router_rule_open_ports list" do
      Router::Rule::OpenPort.stub(:find) { mock_open_port }
      delete :destroy, :id => "1"
      response.should redirect_to(router_rule_open_ports_url)
    end
  end
